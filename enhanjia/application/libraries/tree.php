	...
	function init($raw) {
		...
		foreach($raw as $k=>$v){
			if($v['reid'] == '0'){
				...
				$this->list_array[$this->key]['url'] = $v['eng_name'];
				$this->key = $this->key+1;
				self::get_next_list($v['id'],$v['eng_name']);
	...
	function get_next_list($id,$url='') {
		...
		foreach($raw as $k=>$v){
			if($v['reid'] == $id){
				$this->list_array[$this->key] = $v;
				$this->list_array[$this->key]['level'] = '';
				$this->list_array[$this->key]['url'] = $url.'/'.$v['eng_name'];
				for($i=0;$i<$level;$i++){
				...
				/*如果还有子栏目，递归*/
				if($v['ispart'] == 'catalog'){
					self::get_next_list($v['id'],$url.'/'.$v['eng_name']);
