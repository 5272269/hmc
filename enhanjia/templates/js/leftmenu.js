...
function showHide(objname)
{
	...
	var obj = document.getElementById(objname),
	    objsun = document.getElementById('sun'+objname),
        objLen = $('#sun'+objname).find('#'+objname).length,
        $this = $(arguments[1]);
    ...
    if(objLen){
	    ...
	    if(ckstr==null) ckstr = '';
	    ckstrs = ckstr.split(',');
	    var objname_s = objname;
	    objname = objname.replace('items','');
        $('.sitemu').find('ul').hide();
        $('.sitemu li a').removeClass('selected');
	    // 找到所有的同级样式 关闭
	    if(obj.style.display == 'block' || obj.style.display =='')
	    {
            console.log('执行');
		    $(".sitem").hide(); //隐藏所有下拉子菜单
		    obj.style.display = 'none'; //隐藏自己
