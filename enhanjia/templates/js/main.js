...
function uploadNewFile(f, imgdid, frname, hpos, acname)
{
	var newobj = null;
	if(f.value=='') return ;
	vImg = $Obj(imgdid);
	...
	nForm.action = HTTPURL+"/info/archives_newdo.php?type="+imgdid;
	nForm.target = frname;
	...
	if(!newobj)
	{
		...
		newobj.innerHTML = '<img src="/static/administrator/images/loadinglit.gif" width="16" height="16" alit="" />上传中...';
	}
	newobj.style.display = 'block';
	//提交后还原form的action等参数
	nForm.action = acname;
...
function SeePicNew(f, imgdid, frname, hpos, acname)
{
	...
	nForm.action = HTTPURL+"/info/archives_do.php?type="+imgdid;
	nForm.target = frname;
	nForm.dopost.value = 'uploadLitpic';
	nForm.submit();
...
function LoadQuickDiv(e, surl, oname, w, h)
{
	...
	newobj.innerHTML = '<div style="margin-top:10px;margin-left:10px;"><img src="/static/administrator/images/loadinglit.gif" /> Loading...</div>';
	newobj.style.display = 'block';
	var myajax = new DedeAjax(newobj);
	myajax.SendGet(surl);
	fixupPos = false;
